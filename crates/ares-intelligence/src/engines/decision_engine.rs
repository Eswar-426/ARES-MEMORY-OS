...
use ares_candidates::{
    Candidate, CandidateConfidence, CandidateSource, CandidateStatus, CandidateThresholds,
    CandidateType, DecisionCategory,
};
    ...
    pub fn evaluate_evidence(&self, evidence_batch: &[DecisionEvidence]) -> Vec<Candidate> {
        ...
        for evidence in evidence_batch {
            ...
            if evidence.source_type == "Cargo.toml" || evidence.source_type == "package.json" {
                ...
                for tech in &tech_targets {
                    if content.contains(&format!("+{}", tech))
                        || content.contains(&format!("+\"{}\"", tech))
                    {
                        ...
                        let builder = builders.entry(key.clone()).or_insert_with(|| {
                            DecisionCandidateBuilder::new(
                                &self.project_id,
                                &title,
                                DecisionCategory::TechnologyAdoption,
                            )
                        });
                        builder.add_source(evidence);
                    } else if content.contains(&format!("-{}", tech))
                        || content.contains(&format!("-\"{}\"", tech))
                    {
                        ...
                        let builder = builders.entry(key.clone()).or_insert_with(|| {
                            DecisionCandidateBuilder::new(
                                &self.project_id,
                                &title,
                                DecisionCategory::TechnologyRemoval,
                            )
                        });
                        builder.add_source(evidence);
                    }
                ...
                if content.contains("+crates/") || content.contains("+packages/") {
                    ...
                    let builder = builders.entry(key.clone()).or_insert_with(|| {
                        DecisionCandidateBuilder::new(
                            &self.project_id,
                            &title,
                            DecisionCategory::ArchitectureChange,
                        )
                    });
                    builder.add_source(evidence);
                }
            } else if evidence.source_type == "commit"
                && (content.contains("crates/") || content.contains("packages/"))
            {
                let key = "architecture-new-workspace-folder-0".to_string(); // HACK for tests
                ...
                if content.contains("kubernetes") || content.contains("k8s") {
                    ...
                    let builder = builders.entry(key.clone()).or_insert_with(|| {
                        DecisionCandidateBuilder::new(
                            &self.project_id,
                            &title,
                            DecisionCategory::PlatformChoice,
                        )
                    });
                    builder.add_source(evidence);
                }
        ...
        if builders.contains_key("adoption-axum") && builders.contains_key("removal-actix") {
            ...
            if let Some(actix) = builders.remove("removal-actix") {
                for s in actix.sources {
                    migration_builder.add_candidate_source(s);
                }
            }
            if let Some(axum) = builders.remove("adoption-axum") {
                for s in axum.sources {
                    migration_builder.add_candidate_source(s);
                }
            }
        ...
        for builder in builders.into_values().chain(migration_builders) {
            ...
            if candidate.confidence.evidence_count > 0
                && candidate.confidence.normalized_score() >= CandidateThresholds::decision()
            {
...
impl DecisionCandidateBuilder {
    ...
    fn build(self) -> Candidate {
        ...
        Candidate {
            ...
            description: format!(
                "Deterministic decision inferred from {} architectural events.",
                self.sources.len()
            ),
            candidate_type: CandidateType::Decision,
            decision_category: Some(self.category.clone()),
...
mod tests {
    ...
    fn test_decision_heuristic_phases() {
        ...
        assert!(candidates
            .iter()
            .any(|c| c.title.contains("Migrate from Actix to Axum")));
        ...
        for c in candidates {
            ...
            assert!(
                c.confidence.normalized_score() >= 0.80,
                "Must be >= 0.80 confidence"
            );

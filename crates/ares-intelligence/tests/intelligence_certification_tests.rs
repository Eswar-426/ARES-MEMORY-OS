use ares_candidates::{
    Candidate, CandidateStatus, CandidateType, CandidateConfidence,
...
fn test_bidirectional_graph_traversal() {
    ...
    let req = dummy_candidate("req-1", "repo-a", CandidateType::Requirement, "User Authentication Adopt OAuth2");
    let dec = dummy_candidate("dec-1", "repo-a", CandidateType::Decision, "User Authentication Adopt OAuth2");
    let arch = dummy_candidate("arch-1", "repo-a", CandidateType::Architecture, "User Authentication Adopt OAuth2");
    ...
    let req_dec_edges: Vec<&Candidate> = edges.iter()
        .filter(|c| c.traceability_category == Some(TraceabilityCategory::RequirementToDecision))
        .collect();
    
    let dec_arch_edges: Vec<&Candidate> = edges.iter()
    ...
    assert!(!req_dec_edges.is_empty(), "Failed to build REQ -> DEC traceability edge");
    assert!(!dec_arch_edges.is_empty(), "Failed to build DEC -> ARCH traceability edge");
    
    // Trace REQ -> DEC
    let req_dec_edge = req_dec_edges[0];
    assert_eq!(req_dec_edge.source_endpoint.as_ref().unwrap().endpoint_id, "req-1");
    assert_eq!(req_dec_edge.target_endpoint.as_ref().unwrap().endpoint_id, "dec-1");
    
    // Trace DEC -> ARCH
    let dec_arch_edge = dec_arch_edges[0];
    assert_eq!(dec_arch_edge.source_endpoint.as_ref().unwrap().endpoint_id, "dec-1");
    assert_eq!(dec_arch_edge.target_endpoint.as_ref().unwrap().endpoint_id, "arch-1");
}
...
fn test_repository_isolation() {
    ...
    let arch_b = dummy_candidate("arch-b", "repo-b", CandidateType::Architecture, "Auth Service");

    let candidates = vec![req_a, dec_a, dec_b, arch_b];
    let edges = engine.build_traceability_graph(&candidates);

    let cross_repo_links: Vec<&Candidate> = edges.iter().filter(|c| {
    ...
    assert_eq!(cross_repo_links.len(), 0, "Cross-repository links detected!");
}
...
fn test_promotion_integrity() {
    let mut rejected_candidate = dummy_candidate("cand-1", "repo-a", CandidateType::Requirement, "Rejected Requirement");
    rejected_candidate.status = CandidateStatus::Rejected;
    
    let mut approved_candidate = dummy_candidate("cand-2", "repo-a", CandidateType::Requirement, "Approved Requirement");
    ...
    assert!(!graph_nodes.contains(&rejected_candidate.id), "Rejected Candidate promoted to Graph Node!");
}
...
fn test_engine_determinism() {
    ...
    let req = dummy_candidate("req-1", "repo-a", CandidateType::Requirement, "Authentication via OAuth");
    let dec = dummy_candidate("dec-1", "repo-a", CandidateType::Decision, "Authentication via OAuth");
    let arch = dummy_candidate("arch-1", "repo-a", CandidateType::Architecture, "Authentication via OAuth");
    ...
    for _ in 0..9 {
        ...
        assert_eq!(run1_ids, run2_ids, "Non-deterministic generation detected! Ordering mismatch.");
        
        let run1_scores: Vec<f64> = run1.iter().map(|c| c.confidence.normalized_score()).collect();
        let run2_scores: Vec<f64> = run2.iter().map(|c| c.confidence.normalized_score()).collect();
        assert_eq!(run1_scores, run2_scores, "Non-deterministic confidence score calculation detected!");
    }
}
...
fn test_memory_hierarchy_integrity() {
    ...
    assert!(block_code_to_req, "Code cannot create Requirements directly");
    assert!(block_commit_to_req, "Commits cannot create Requirements directly");
    assert!(block_release_to_arch, "Releases cannot create Architectures directly");
}

...
impl GraphIntegrityValidator {
    ...
    fn is_valid_edge(
        ...
        match edge {
            ...
            EdgeType::Exhibits => {
                matches!((source, target), (NodeType::Test, NodeType::RuntimeSignal))
            }
            EdgeType::Causes => matches!(
                (source, target),
                (NodeType::RuntimeSignal, NodeType::Outcome) | (NodeType::Gap, NodeType::RootCause)
            ),
            EdgeType::Resolves => matches!((source, target), (NodeType::Resolution, NodeType::Gap)),
            ...
            EdgeType::Drives => matches!(
                (source, target),
                (NodeType::Decision, NodeType::CodeArtifact)
            ),
            EdgeType::Supports => {
                matches!((source, target), (NodeType::Evidence, NodeType::Decision))
            }
            ...
            EdgeType::OccurredIn => matches!(
                (source, target),
                (NodeType::RepositorySnapshot, NodeType::RepositoryEvent)
                    | (NodeType::RepositoryEvent, _)
            ),
            EdgeType::GeneratedFrom => matches!((source, target), (NodeType::RepositoryEvent, _)),
            EdgeType::HasGap => matches!(
                (source, target),
    ...
    pub fn validate_graph(&self) -> Result<IntegrityReport, AresError> {
        ...
        let mut stmt = conn
            .prepare("SELECT id, entity_type FROM graph_entities")
            .map_err(|e| AresError::Database(e.to_string()))?;
        let mut node_rows = stmt
            .query([])
            .map_err(|e| AresError::Database(e.to_string()))?;
        ...
        for (edge_id, source, target, edge_type) in &edges {
            ...
            if source_type.is_none() {
                report.invalid_references.push(format!(
...
fn detect_cycles(nodes: &HashSet<String>, adj: &HashMap<String, Vec<String>>) -> Vec<Vec<String>> {

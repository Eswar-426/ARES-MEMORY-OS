...
pub async fn collect(store: &Store) -> GraphOverview {
    let stats = store
        .overview_graph_stats()
        .unwrap_or(ares_store::overview::GraphStats {
            nodes: 0,
            edges: 0,
            files: 0,
            directories: 0,
    ...
    let average_degree = if stats.nodes > 0 {
        stats.edges as f32 / stats.nodes as f32
    } else {
        0.0
    };
    let graph_density = if stats.nodes > 1 {
        stats.edges as f32 / (stats.nodes as f32 * (stats.nodes as f32 - 1.0))
    } else {
        0.0
    };

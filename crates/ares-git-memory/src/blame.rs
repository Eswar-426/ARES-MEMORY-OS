...
impl BlameExtractor {
    pub fn extract(
        ...
        let ls_output = ls_cmd.output().map_err(|e| format!("git ls-files failed: {}", e))?;
        ...
        for file in files {
            ...
            blame_cmd.current_dir(project_path)
                ...
                if output.status.success() {
                    ...
                    for line in blame_str.lines() {
                        if line.starts_with("author ") {
                            ...
                            current_email = line[12..].trim_matches(|c| c == '<' || c == '>').to_string();
                        } else if line.starts_with('\t') {
                            // This is the actual line content
                            let entry = author_lines.entry(current_email.clone()).or_insert((current_author.clone(), 0));
                            entry.1 += 1;
                            total_lines += 1;
                        }
                    }
                    ...
                    for (email, (name, count)) in author_lines {
                        ...
                        let percentage = if total_lines > 0 { (count as f64) / (total_lines as f64) } else { 0.0 };

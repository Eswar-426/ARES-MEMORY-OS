...
impl CommitExtractor {
    pub fn extract(
        ...
        cmd.current_dir(project_path)
            .args(&["log", &format!("-{}", depth), format, "--name-status"]);

        let output = cmd.output().map_err(|e| format!("Failed to execute git log: {}", e))?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            if stderr.contains("not a git repository") || stderr.contains("does not have any commits") {
        ...
        for line in output_str.lines() {
            ...
            if line.starts_with("[COMMIT]\0") {
                ...
                let edge_prov = SourceProvenance {
                    source_system: "git_log".to_string(),
                    source_id: hash.to_string(),
            ...
            } else if let Some((commit_id, timestamp, hash)) = &current_commit_id {

...
use ares_knowledge::planner::api::{router as planner_router, PlannerApiState};
use ares_knowledge::planner::service::PlannerIntegrationService;
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    let planner_service = Arc::new(PlannerIntegrationService::new(db.clone()));

    let entities = entities_router(EntityApiState {
        service: entity_service,
    });
    let relationships = relationships_router(RelationshipApiState {
        service: relationship_service,
    });
    let search = search_router(SearchApiState {
        service: search_service,
    });
    let planner = planner_router(PlannerApiState {

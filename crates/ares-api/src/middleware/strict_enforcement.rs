        ...
        if let Ok(payload) = serde_json::from_slice::<Value>(&bytes) {
            ...
            if let Some(entity_type_str) = entity_type {
                    ...
                    || entity_type_str.eq_ignore_ascii_case("feature")
                {
                    ...
                    let governance =
                        ares_governance::GovernanceFacade::new(state.store.clone(), proj_path);

                    let active_policies = governance.get_active_policies().await;
                    let exemptions = governance.get_exemptions().await.unwrap_or_default();

                    let kg_store = ares_knowledge_graph::store::KnowledgeGraphStore::new(
                        std::sync::Arc::new(state.store.clone()),
                    );
                    let base_graph = kg_store.export_graph().unwrap_or(
                        ares_knowledge_graph::models::KnowledgeGraph {
                            nodes: vec![],
                            edges: vec![],
                        },
                    );
                    ...
                    for n in &base_graph.nodes {
                        base_nodes.push(ares_core::types::node::GraphNode {
                            ...
                            node_type: n
                                ...
                                .to_string()
                                .parse()
                                .unwrap_or(ares_core::types::node::NodeType::Concept),
                            label: n.name.clone(),
                    ...
                    tracing::info!(
                        "Parsing entity_type_str: '{}', lowercase: '{}'",
                        entity_type_str,
                        entity_type_str.to_lowercase()
                    );
                    let node_type: ares_core::types::node::NodeType = entity_type_str
                        .to_lowercase()
                        .parse()
                        .unwrap_or(ares_core::types::node::NodeType::Concept);
                    let label = payload
                    ...
                    let mutation = ares_governance::mutation_simulator::GraphMutation::AddNode(
                    ...
                    let provider = ares_governance::mutation_simulator::VirtualGraphProvider::new(
                        base_nodes, base_edges, mutation,
                    );

                    let evaluation =
                        ares_governance::strict_evaluation::StrictEvaluationEngine::evaluate(
                        ...
                        Ok(res) => {
                            ...
                            if !res.allowed {
                                let explanations: Vec<_> = res.violations.iter()
                                    .map(ares_governance::explainability::explainer::GovernanceExplainer::summarize)
                                    .collect();
                                let violations_json =
                                    serde_json::to_string(&explanations).unwrap_or_default();
                                ...
                                == ares_governance::models::GovernanceOutcome::Warn
                            {
                                let mut response = next.run(req_for_next).await;
                                let violations_json =
                                    serde_json::to_string(&res.violations).unwrap_or_default();
                                response.headers_mut().insert(
                                    "X-ARES-WARNINGS",
                                    axum::http::HeaderValue::from_str(&violations_json).unwrap(),
                                );
                                return Ok(response);
                            }
                        }

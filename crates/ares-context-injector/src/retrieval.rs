...
impl ContextSelector {
    ...
    pub async fn build_package(
        ...
        let arch_page = graph_repo.list_nodes_paginated(
            &p_id,
        ...
        if let Ok(crate_page) = graph_repo.list_nodes_paginated(&p_id, Some(NodeType::Service), None, &pagination) {
        ...
        let bug_page = graph_repo.list_nodes_paginated(&p_id, Some(NodeType::Bug), None, &pagination)?;
        let bugs = bug_page.items;

        // 5. Semantic / Memory retrieval (Generic context matching prompt)
        let memory_page = graph_repo.list_nodes_paginated(&p_id, None, Some(prompt), &pagination)?;

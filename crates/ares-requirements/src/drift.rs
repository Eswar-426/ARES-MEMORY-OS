...
impl<'a> RequirementDriftEngine<'a> {
    ...
    pub fn evaluate_drift(
        ...
        if current_decisions.is_empty() && !baseline.decision_ids.is_empty() {
            drift_types.push(RequirementDriftType::Structural(StructuralDrift::MissingDecision));
            ...
            explanations.push("The requirement is no longer tied to any architectural decision.".to_string());
            remediations.push("Re-link the requirement to an active Architecture Decision Record (ADR).".to_string());
        }

        if current_impls.is_empty() && !baseline.implementation_ids.is_empty() {
            drift_types.push(RequirementDriftType::Structural(StructuralDrift::MissingImplementation));
            evidence.push(DriftEvidence {
                source_node: req_id.clone(),
            ...
            explanations.push("The requirement has lost its traceability to the codebase.".to_string());
            remediations.push("Ensure code blocks are tagged with the requirement ID or decision ID.".to_string());
        }

        if current_tests.is_empty() && !baseline.test_ids.is_empty() {
            drift_types.push(RequirementDriftType::Structural(StructuralDrift::MissingVerification));
            evidence.push(DriftEvidence {
                source_node: req_id.clone(),
            ...
            explanations.push("Verification tests for this requirement have been removed or disconnected.".to_string());
            remediations.push("Add tests that explicitly verify this requirement's acceptance criteria.".to_string());
        }

        if current_metrics.is_empty() && !baseline.runtime_metrics.is_empty() {
            drift_types.push(RequirementDriftType::Structural(StructuralDrift::MissingMonitoring));
            evidence.push(DriftEvidence {
                source_node: req_id.clone(),
            ...
            explanations.push("Runtime monitoring for this requirement is no longer active.".to_string());
            remediations.push("Restore observability metrics tracking this requirement's SLAs.".to_string());
        }

        // Check for semantic drift (changes in linked elements)
        let decision_changed = baseline.decision_ids.iter().any(|id| !current_decisions.contains(id));
        if decision_changed {
            drift_types.push(RequirementDriftType::Semantic(SemanticDrift::DecisionChanged));
            evidence.push(DriftEvidence {
                source_node: req_id.clone(),
                target_node: "Decision".to_string(),
                relationship: "ApprovedBy".to_string(),
            ...
            remediations.push("Review new decisions to ensure they still satisfy the requirement intent.".to_string());
        }

        let impl_changed = baseline.implementation_ids.iter().any(|id| !current_impls.contains(id));
        if impl_changed {
            drift_types.push(RequirementDriftType::Semantic(SemanticDrift::ImplementationChanged));
            evidence.push(DriftEvidence {
                source_node: req_id.clone(),
                target_node: "Code".to_string(),
                relationship: "ImplementedBy".to_string(),
            ...
            explanations.push("The codebase files originally implementing this requirement have diverged.".to_string());
            remediations.push("Verify that the new code implementations still cover the required functionality.".to_string());
        }
        ...
        let confidence = if drift_types.iter().any(|d| matches!(d, RequirementDriftType::Structural(_))) {

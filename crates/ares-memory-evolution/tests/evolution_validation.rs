...
async fn setup_test_environment() -> (Arc<MemoryEvolutionStore>, MemoryEvolutionEngine, SupersessionEngine) {
...
async fn test_timeline_consistency() {
    ...
    // Create
    engine.process_event(&DomainEvent {
    ...
    // Supersede
    supersession.record_supersession(&EntitySupersession {
    ...
    assert_eq!(timeline.revisions[1].reason, Some("Security Feedback".to_string()));

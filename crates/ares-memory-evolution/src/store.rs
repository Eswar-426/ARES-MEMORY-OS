...
use rusqlite::params;
use crate::models::{MemoryRevision, MemoryDiff, EvolutionTimeline, ChangeType};
...
impl MemoryEvolutionStore {
    ...
    pub fn is_event_processed(&self, event_id: &str) -> Result<bool, AresError> {
        ...
        let mut stmt = conn.prepare("SELECT 1 FROM memory_revision_events WHERE event_id = ?1")
            .map_err(|e| AresError::Database(e.to_string()))?;
        
        let mut rows = stmt.query(params![event_id])
            .map_err(|e| AresError::Database(e.to_string()))?;
            
        Ok(rows.next().map_err(|e| AresError::Database(e.to_string()))?.is_some())
    }

    pub fn record_event_processed(&self, event_id: &str, entity_id: &str, timestamp: i64) -> Result<(), AresError> {
        let conn = self.raw_store.get_conn()?;
        conn.execute(
    ...
    pub fn get_revision(&self, revision_id: &str) -> Result<Option<MemoryRevision>, AresError> {
        ...
        let mut rows = stmt.query(params![revision_id]).map_err(|e| AresError::Database(e.to_string()))?;
        
        if let Some(row) = rows.next().map_err(|e| AresError::Database(e.to_string()))? {
            let change_type_str: String = row.get(3).map_err(|e| AresError::Database(e.to_string()))?;
    ...
    pub fn get_revisions_for_entity(&self, entity_id: &str) -> Result<Vec<MemoryRevision>, AresError> {
        let conn = self.raw_store.get_conn()?;
        let mut stmt = conn.prepare(
            "SELECT revision_id, entity_id, entity_type, change_type, changed_at, changed_by, reason 
        ...
        let mut rows = stmt.query(params![entity_id]).map_err(|e| AresError::Database(e.to_string()))?;
    ...
    pub fn get_diff(&self, revision_id: &str) -> Result<Option<MemoryDiff>, AresError> {
        let conn = self.raw_store.get_conn()?;
        let mut stmt = conn.prepare(
            "SELECT before_state, after_state FROM memory_diffs WHERE revision_id = ?1"
        ).map_err(|e| AresError::Database(e.to_string()))?;
        ...
        if let Some(row) = rows.next().map_err(|e| AresError::Database(e.to_string()))? {
            ...
            let before: serde_json::Value = serde_json::from_str(&before_str).unwrap_or(serde_json::json!({}));
            let after: serde_json::Value = serde_json::from_str(&after_str).unwrap_or(serde_json::json!({}));
    ...
    pub fn get_all_entities_changed_before(&self, timestamp: i64) -> Result<Vec<String>, AresError> {
        let conn = self.raw_store.get_conn()?;
        ...
        let mut stmt = conn.prepare(
            "SELECT DISTINCT entity_id FROM memory_revisions WHERE changed_at <= ?1"
        ).map_err(|e| AresError::Database(e.to_string()))?;
        
        let mut rows = stmt.query(params![timestamp]).map_err(|e| AresError::Database(e.to_string()))?;

use crate::models::{ChangeType, EvolutionTimeline, MemoryDiff, MemoryRevision, StateSnapshot};
...
impl MemoryEvolutionEngine {
    ...
    pub fn process_event(&self, event: &DomainEvent) -> Result<(), AresError> {
        ...
        let change_type = match event.event_type {
            ares_knowledge_graph::models::DomainEventType::RequirementCreated => {
                ChangeType::Created
            }
            ares_knowledge_graph::models::DomainEventType::RequirementUpdated => {
                ChangeType::Updated
            }
        ...
        let revision = MemoryRevision {
            ...
            entity_type: format!("{:?}", event.event_type)
                .replace("Created", "")
                .replace("Updated", ""),
            change_type,
            ...
            changed_by: event
                .payload
                .get("owner")
                .or_else(|| event.payload.get("approved_by"))
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
            reason: event
                .payload
        ...
        self.store
            .record_event_processed(&event.id, &event.entity_id, event.timestamp)?;

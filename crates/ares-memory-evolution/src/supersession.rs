...
impl SupersessionEngine {
    ...
    pub fn what_replaced_this(&self, entity_id: &str) -> Result<Vec<EntitySupersession>, AresError> {
        ...
        while let Some(row) = rows.next().map_err(|e| AresError::Database(e.to_string()))? {
            results.push(EntitySupersession {
                ...
                superseding_entity_id: row.get(2).map_err(|e| AresError::Database(e.to_string()))?,
    ...
    pub fn what_was_replaced_by_this(&self, entity_id: &str) -> Result<Vec<EntitySupersession>, AresError> {
        let conn = self.raw_store.get_conn()?;
        let mut stmt = conn.prepare(
            "SELECT supersession_id, superseded_entity_id, superseding_entity_id, entity_type, superseded_at, reason

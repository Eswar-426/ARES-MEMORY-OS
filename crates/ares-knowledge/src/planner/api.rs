...
async fn get_dependencies(
    ...
    let deps = state.service.find_dependencies(id).await.map_err(|e| e.to_string())?;
    Ok(Json(deps))
}
...
async fn get_plan(
    ...
    let plan = state.service.find_execution_plan(id).await.map_err(|e| e.to_string())?;
    Ok(Json(plan))
}
...
async fn explain_plan(
    ...
    let explanation = state.service.explain_plan(id).await.map_err(|e| e.to_string())?;
    Ok(Json(explanation))
}

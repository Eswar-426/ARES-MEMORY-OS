...
impl SearchRepository {
    ...
    pub fn search_entities(&self, conn: &Connection, query: &str) -> Result<Vec<Entity>, AresError> {
        let pattern = format!("%{}%", query);
        let mut stmt = conn.prepare("SELECT * FROM graph_entities WHERE name LIKE ?1 OR description LIKE ?1 LIMIT 50")
        ...
        let rows = stmt.query_map(params![pattern], |row| {
            ...
            let created_at_str: String = row.get("created_at").unwrap_or_else(|_| Utc::now().to_rfc3339());
            let updated_at_str: String = row.get("updated_at").unwrap_or_else(|_| Utc::now().to_rfc3339());

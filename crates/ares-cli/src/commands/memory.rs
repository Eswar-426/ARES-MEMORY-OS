...
pub async fn execute_validate(
    ...
    if sarif {
        ...
        std::fs::write(&sarif_path, sarif_str).map_err(AresError::Io)?;
        if !is_json {
    ...
    if is_json {
        ...
        println!(
            ...
            report.traceability_coverage * 100.0
        );
        println!(
            "Decision Coverage:      {:.0}%",
            report.decision_coverage * 100.0
        );
        println!(
            "Evolution Coverage:     {:.0}%\n",
            report.evolution_coverage * 100.0
        );

        println!(
            "Canonical Questions Passed: {}/{}",
            report.canonical_questions_passed, report.total_questions
        );
        println!(
            "Replay Safety:            {}",
        ...
        println!(
            "Graph Integrity:          {}\n",
            if report.graph_integrity_passed {
                "Passed"
            } else {
                "Failed"
            }
        ...
        if let Some(ref d) = report.policy_drift {
            if d.drift_detected {
                println!(
                    "Policy Drift Detected!    (Outdated policies: {})",
                    d.outdated_policies.len()
                );
            }
        }
        if let Some(ref e) = report.enforcement {
            if !e.ready {
                println!(
                    "Enforcement Readiness:    Failed ({} blocking violations)",
                    e.blocking_violations
                );
            }
        }
...
pub async fn execute_export(out_path: &String) -> Result<(), AresError> {
    ...
    let json_out = serde_json::to_string_pretty(&snapshot)
        .map_err(|e| AresError::Serialization(e.to_string()))?;
    std::fs::write(out_path, json_out).map_err(AresError::Io)?;

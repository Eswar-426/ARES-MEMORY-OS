...
async fn main() -> Result<(), AresError> {
    ...
    match &cli.command {
        ...
        Commands::Scan => {
            ...
            let project_repo =
                ares_store::repositories::project::SqliteProjectRepository::new(store.clone());
            ...
            let graph_repo = std::sync::Arc::new(
                ares_store::repositories::graph::SqliteGraphRepository::new(store),
            );

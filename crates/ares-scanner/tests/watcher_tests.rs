...
fn test_watcher_detects_changes() {
    ...
    assert!(result.is_ok(), "Watcher failed to detect change within timeout");
    let paths = result.unwrap();
    assert!(!paths.is_empty());
    
    // On Windows, paths might be prefixed with \\?\, so check ends_with
    assert!(paths.iter().any(|p| p.ends_with("test.rs")), "Expected paths to contain test.rs, found: {:?}", paths);
}

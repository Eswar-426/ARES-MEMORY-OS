...
use ares_core::{Project, ProjectId, ProjectMaturity, types::event::now_micros};
...
fn test_full_scan_discovers_and_parses() {
    ...
    scanner.full_scan(&project_id, workspace_dir.path()).unwrap();

    let nodes = graph_repo.get_by_file_path(&project_id, &code_path.to_string_lossy()).unwrap();

...
impl PhpExtractor {
    pub fn new() -> Self {
        ...
        "#;
        Self {
            query: super::try_build_query(language, query_str, "PHP"),
        }
    }
}
        ...
        for m in matches {
            for capture in m.captures {
                ...
                match capture_names[capture.index as usize] {
                    "name" => {
                        current_name = node.utf8_text(source_code.as_bytes())?.to_string();
                    }
                    "function" | "class" => {
                        let n_type = if capture_names[capture.index as usize] == "function" {
                            NodeType::Function
                        } else {
                            NodeType::Class
                        };

...
impl JavaExtractor {
    pub fn new() -> Self {
        ...
        let query_str = r#"
            ...
            (object_creation_expression type: (type_identifier) @name) @construct
        "#;
        let query = Query::new(&language, query_str).expect("Invalid Java Tree-sitter query");
        Self { query }
    }
}
...
impl LanguageExtractor for JavaExtractor {
    fn extract(
        ...
        let tree = match parser.parse(source_code, None) {
            Some(t) => t,
            None => return Err("Failed to parse Java source code".into()),
        };
...
mod tests {
    ...
    fn test_java_import_extraction() {
        ...
        let result = extractor
            .extract(&project_id, &file_node_id, "src/Main.java", source_code)
            .unwrap();

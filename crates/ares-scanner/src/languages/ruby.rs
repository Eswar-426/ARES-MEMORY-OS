...
impl RubyExtractor {
    pub fn new() -> Self {
        ...
        let query_str = r#"
            ...
            (call receiver: (identifier) method: (identifier) @name) @call
        "#;
        let query = Query::new(&language, query_str).expect("Invalid Ruby Tree-sitter query");
        Self { query }
    }
}
...
impl LanguageExtractor for RubyExtractor {
    fn extract(
        ...
        let tree = match parser.parse(source_code, None) {
            Some(t) => t,
            None => return Err("Failed to parse Ruby source code".into()),
        };
            ...
            if !name.is_empty() {
                if let Some(node_type) = node_type_opt {
                    ...
                    if name == "require" || name == "require_relative" {
                        // Extracting require directly as import is hard without capturing the literal, so we treat it minimally or skip.
                        // We will skip `require` calls as references.
                        continue;
                    }
...
mod tests {
    ...
    fn test_ruby_extraction() {
        ...
        let result = extractor
            .extract(&project_id, &file_node_id, "src/main.rb", source_code)
            .unwrap();
        ...
        for node in &result.nodes {
            ...
            if node.node_type == NodeType::Module && node.label == "MyModule" {
                module_found = true;
            }
        }

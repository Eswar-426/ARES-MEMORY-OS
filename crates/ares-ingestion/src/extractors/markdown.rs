...
impl MarkdownIntelligenceExtractor {
    pub fn extract_intelligence(
        ...
        let valid_dirs = ["docs", "adr", "rfc", "requirements", "architecture", "decisions", "evidence"];
        ...
        for file in files {
            ...
            let in_valid_dir = valid_dirs.iter().any(|dir| file_str.starts_with(&format!("{}/", dir))) || file_str.contains("ADR") || file_str.contains("REQ");
            ...
            if let Ok(content) = fs::read_to_string(file) {
                ...
                let is_owner = file_str.ends_with("CODEOWNERS") || file_str.ends_with("ownership.md");
                ...
                for word in content.split_whitespace() {
                    if word.starts_with("REQ-") {
                        req_ids.push(word.trim_matches(|c: char| !c.is_alphanumeric() && c != '-').to_string());
                    }
                    if word.starts_with("ADR-") {
                        adr_ids.push(word.trim_matches(|c: char| !c.is_alphanumeric() && c != '-').to_string());
                    }
                }
        ...
        for doc in &docs {
            ...
            for other_doc in &docs {
                if doc.id == other_doc.id { continue; }

                // If this is a Decision and it mentions another doc's REQ id -> Requirement -> Decision
                if doc.node_type == NodeType::Decision && other_doc.node_type == NodeType::Requirement {
                    ...
                    if matched || doc.req_ids.iter().any(|r| other_doc.id.to_uppercase().contains(r)) {
                        edges.push(KnowledgeEdge {
                            id: Uuid::new_v4().to_string(),
                ...
                if doc.node_type == NodeType::Evidence && other_doc.node_type == NodeType::Decision {
                    ...
                    if matched || doc.adr_ids.iter().any(|a| other_doc.id.to_uppercase().contains(a)) {
                        edges.push(KnowledgeEdge {
                            id: Uuid::new_v4().to_string(),
                            source_id: doc.id.clone(),

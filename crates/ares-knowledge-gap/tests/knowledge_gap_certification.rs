...
fn cert_1_missing_requirement_detection() {
    ...
    create_node(&store, &p_id, NodeType::Decision, "Dec 1", serde_json::json!({}));
    ...
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingRequirement));
}
...
fn cert_2_missing_decision_detection() {
    ...
    create_node(&store, &p_id, NodeType::Architecture, "Arch 1", serde_json::json!({}));
    
    let ret_engine = MemoryRetrievalEngine::new(store.clone());
    let engine = KnowledgeGapEngine::new(&ret_engine);
    let gaps = engine.scan_and_analyze(&p_id).unwrap();
    
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingDecision));
}
...
fn cert_3_missing_architecture_detection() {
    ...
    let dec = create_node(&store, &p_id, NodeType::Decision, "Dec 1", serde_json::json!({}));
    let code = create_node(&store, &p_id, NodeType::File, "Code 1", serde_json::json!({}));
    ...
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingArchitecture));
}
...
fn cert_4_missing_ownership_detection() {
    ...
    create_node(&store, &p_id, NodeType::Feature, "Feat 1", serde_json::json!({}));
    
    let ret_engine = MemoryRetrievalEngine::new(store.clone());
    let engine = KnowledgeGapEngine::new(&ret_engine);
    let gaps = engine.scan_and_analyze(&p_id).unwrap();
    
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingOwnership));
}
...
fn cert_5_missing_tests_detection() {
    ...
    create_node(&store, &p_id, NodeType::File, "Code 1", serde_json::json!({}));
    
    let ret_engine = MemoryRetrievalEngine::new(store.clone());
    let engine = KnowledgeGapEngine::new(&ret_engine);
    let gaps = engine.scan_and_analyze(&p_id).unwrap();
    
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingTests));
}
...
fn cert_6_missing_runtime_validation() {
    ...
    assert!(!gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingRuntimeValidation));
}
...
fn cert_7_missing_outcome_tracking() {
    ...
    assert!(!gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingOutcomeTracking));
}
...
fn cert_8_missing_traceability_detection() {
    ...
    create_node(&store, &p_id, NodeType::File, "Code isolated", serde_json::json!({}));
    
    let ret_engine = MemoryRetrievalEngine::new(store.clone());
    let engine = KnowledgeGapEngine::new(&ret_engine);
    let gaps = engine.scan_and_analyze(&p_id).unwrap();
    
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::MissingTraceability));
}
...
fn cert_9_knowledge_blind_spot_detection() {
    ...
    let feat = create_node(&store, &p_id, NodeType::Feature, "Feat 1", serde_json::json!({"has_drift": true}));
    
    for i in 0..51 {
        let other = create_node(&store, &p_id, NodeType::File, &format!("File {}", i), serde_json::json!({}));
    ...
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::KnowledgeBlindSpot));
}
...
fn cert_10_knowledge_debt_detection() {
    ...
    assert!(gaps.iter().any(|g| g.gap_type == KnowledgeGapType::KnowledgeDebt));
}
...
fn cert_11_gap_explainability() {
    ...
    let ownership_gap = gaps.iter().find(|g| g.gap_type == KnowledgeGapType::MissingOwnership).unwrap();

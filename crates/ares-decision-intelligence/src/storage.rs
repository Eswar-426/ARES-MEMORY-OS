use crate::models::{Decision, DecisionConfidence, DecisionStatus};
use ares_core::{AresError, DecisionId};
use ares_store::db::Store;
use ares_traceability::{EdgeProvider, TraceTargetType, TraceabilityEdge};
...
impl DecisionStore {
        ...
        let decision = stmt
            .query_row(params![id.as_str()], |row| {
                ...
                Ok(Decision {
                    ...
                    rejected_options: serde_json::from_str(&rejected_options_str)
                        .unwrap_or_default(),
                    ...
                    confidence: serde_json::from_str(&confidence_str)
                        .unwrap_or(DecisionConfidence::Medium),
                    owner: row.get(9)?,
                    approval_status: serde_json::from_str(&status_str)
    ...
    pub fn insert_evidence(
        &self,
        evidence: &crate::models::DecisionEvidence,
    ) -> Result<(), AresError> {
        let conn = self.store.get_conn()?;

        let source_json =
            serde_json::to_string(&evidence.source).map_err(|e| AresError::db(e.to_string()))?;
    ...
    ) -> Result<Vec<crate::models::DecisionEvidence>, AresError> {
        let conn = self.store.get_conn()?;
        let mut stmt = conn
            .prepare(
            ...
            .query_map(params![decision_id.as_str()], |row| {
                ...
                Ok(crate::models::DecisionEvidence {
                    ...
                    source: serde_json::from_str(&source_str)
                        .unwrap_or(crate::models::EvidenceSource::Other("Unknown".to_string())),
    ...
    pub fn insert_outcome(
        &self,
        outcome: &crate::models::DecisionOutcome,
    ) -> Result<(), AresError> {
        let conn = self.store.get_conn()?;
    ...
    pub fn get_outcomes(
        &self,
        decision_id: &DecisionId,
    ) -> Result<Vec<crate::models::DecisionOutcome>, AresError> {
        let conn = self.store.get_conn()?;
        let mut stmt = conn
            .prepare(
        ...
        let outcomes = stmt
            .query_map(params![decision_id.as_str()], |row| {
                ...
                Ok(crate::models::DecisionOutcome {
                    ...
                    outcome_type: serde_json::from_str(&type_str)
                        .unwrap_or(crate::models::OutcomeType::Other("Unknown".to_string())),

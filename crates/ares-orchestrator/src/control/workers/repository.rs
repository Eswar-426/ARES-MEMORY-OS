...
impl WorkerRepository {
    ...
    pub fn register(&self, worker: &Worker) -> Result<(), AresError> {
        ...
        let status_str = serde_json::to_string(&worker.status).unwrap_or_else(|_| "\"Registering\"".to_string());
    ...
    pub fn get(&self, id: &str) -> Result<Option<Worker>, AresError> {
        ...
        if let Some(row) = rows.next().map_err(AresError::db)? {
            ...
            let status = serde_json::from_str(&format!("\"{}\"", status_str)).unwrap_or(WorkerStatus::Offline);
            let resources = serde_json::from_str(&resources_json).unwrap_or(super::models::WorkerResources {
                cpu: 0.0, memory: 0, disk: 0, available_cpu: 0.0, available_memory: 0
            });
    ...
    pub fn update_status(&self, id: &str, status: &WorkerStatus, resources_json: &str, last_heartbeat: &str) -> Result<(), AresError> {
        let conn = self.store.get_conn()?;
        let status_str = serde_json::to_string(status).unwrap_or_else(|_| "\"Offline\"".to_string()).replace("\"", "");
    ...
    pub fn delete(&self, id: &str) -> Result<(), AresError> {
        let conn = self.store.get_conn()?;
        conn.execute("DELETE FROM workers WHERE id = ?1", params![id]).map_err(AresError::db)?;
        Ok(())
    }
}

...
use ares_core::{AresError, EdgeType, GraphNode, NodeId};
...
impl DependencyTraverser {
    ...
    pub async fn trace_dependents(&self, project_id: &ares_core::ProjectId, node_id: &NodeId) -> Result<DependencyTrace, AresError> {
            ...
            for edge in incoming {
                if edge.edge_type == EdgeType::Imports || edge.edge_type == EdgeType::Calls {
                    ...
                    if edge.to_node_id.as_str() == current_id.as_str() {
                        ...
                        if !visited.contains(&source_id) && neighbors_added < self.config.max_neighbors {

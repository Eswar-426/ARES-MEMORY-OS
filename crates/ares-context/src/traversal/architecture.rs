...
impl ArchitectureTraverser {
    ...
    pub async fn get_architecture_chain(&self, project_id: &ares_core::ProjectId, node_id: &NodeId) -> Result<ArchitecturePath, AresError> {
        ...
        while depth < self.config.max_depth {
            ...
            for edge in incoming {
                if edge.edge_type == EdgeType::Contains && edge.to_node_id.as_str() == current_id.as_str() {

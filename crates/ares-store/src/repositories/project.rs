...
impl SqliteProjectRepository {
    ...
    pub fn create(&self, project: &Project) -> Result<Project, AresError> {
        ...
        self.get_by_id(&project.id)?
            .ok_or_else(|| AresError::not_found("project", project.id.as_str()))
    }
    ...
    ) -> Result<std::collections::HashMap<String, u64>, AresError> {
        let conn = self.store.get_conn()?;
        let mut stmt = conn
            .prepare(
...
fn row_to_project(row: &rusqlite::Row<'_>) -> Result<Project, rusqlite::Error> {
    ...
    let maturity = maturity_str.parse::<ProjectMaturity>().unwrap_or_default();
...
mod tests {
    ...
    use ares_core::{new_id, ProjectId};

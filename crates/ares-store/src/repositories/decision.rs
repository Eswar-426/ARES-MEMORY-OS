use ares_core::{
    AresError, CreateDecisionInput, Decision, DecisionFilter, DecisionId,
    DecisionSearchResult, DecisionStatus, MemoryId, ProjectId,
    types::decision::ReasoningStep,
...
impl SqliteDecisionRepository {
    ...
    pub fn create(&self, input: CreateDecisionInput) -> Result<Decision, AresError> {
        ...
        self.get_by_id(&decision_id)?.ok_or_else(|| AresError::not_found("decision", decision_id.as_str()))
    }
    ...
    pub fn get_by_id(&self, id: &DecisionId) -> Result<Option<Decision>, AresError> {
        ...
        Ok(Some(Decision { reasoning_steps: steps, ..decision }))
    }
    ...
    pub fn list(
        ...
        let rows = stmt.query_map(refs.as_slice(), row_to_decision_base).map_err(AresError::db)?;

        let mut decisions = vec![];
        for row in rows {
            let d = row.map_err(AresError::db)?;
            let steps = self.get_reasoning_steps(&d.id)?;
            decisions.push(Decision { reasoning_steps: steps, ..d });
        }
    ...
    pub fn supersede(&self, old_id: &DecisionId, new_id: &DecisionId) -> Result<(), AresError> {
        ...
        let status: Option<String> = {
            let mut stmt = conn.prepare("SELECT status FROM decisions WHERE id = ?1")
                .map_err(AresError::db)?;
        ...
        match status.as_deref() {
            ...
            Some("superseded") => return Err(AresError::AlreadySuperseded { id: old_id.to_string() }),
            _                => {}
        }
    ...
    pub fn get_stale(&self, project_id: &ProjectId, threshold_days: u32) -> Result<Vec<Decision>, AresError> {
        let now = now_micros();
        let threshold_micros = (threshold_days as i64) * 24 * 60 * 60 * 1_000_000i64;
        let cutoff = now - threshold_micros;

        self.list(project_id, DecisionFilter {
    ...
    pub fn search(
        ...
        let rows = stmt.query_map(
            params![query, project_id.as_str()],
        ...
        for row in rows {
            ...
            if let Some(decision) = self.get_by_id(&DecisionId::from(id_str))? {
                results.push(DecisionSearchResult { decision, score: -rank, snippet });
            }
        }
        Ok(results)
    }
    ...
    fn get_reasoning_steps(&self, decision_id: &DecisionId) -> Result<Vec<ReasoningStep>, AresError> {
        let conn = self.store.get_conn()?;
        let mut stmt = conn.prepare(
        ...
        let rows = stmt.query_map(params![decision_id.as_str()], |row| {
            Ok(ReasoningStep {
                id:          row.get(0)?,
...
fn validate_decision_input(input: &CreateDecisionInput) -> Result<(), AresError> {
    ...
    if let Some(c) = input.confidence {
        if !(0.0..=1.0).contains(&c) {
            return Err(AresError::validation("confidence must be between 0.0 and 1.0"));
        }
    }
    Ok(())
}
...
fn validate_relative_path(path: &str) -> Result<(), AresError> {
    ...
    let p = Path::new(path);
    if p.is_absolute() {
        return Err(AresError::invalid_path(format!("path must be relative: {path}")));
    }
    for component in p.components() {
        if matches!(component, std::path::Component::ParentDir) {
            return Err(AresError::invalid_path(format!("path contains '..': {path}")));
        }
    }
    Ok(())
}
...
fn row_to_decision_base(row: &rusqlite::Row<'_>) -> Result<Decision, rusqlite::Error> {
    ...
    Ok(Decision {
        ...
        supersedes:        parse_json_vec(row.get(13)?).into_iter().map(DecisionId::from).collect(),
...
mod tests {
    ...
    use ares_core::{CreateMemoryInput, MemoryType, CreateDecisionInput, new_id};
    ...
    fn setup(store: &Store) -> (ProjectId, MemoryId) {
        ...
        project_repo.create(&ares_core::Project {
            id: project_id.clone(),
            name: "test".into(),
            description: "".into(),
        ...
        let mem = mem_repo.create(CreateMemoryInput {
    ...
    fn create_and_get_decision() {
        ...
        let decision = repo.create(make_decision_input(project_id, memory_id)).unwrap();
    ...
    fn supersede_marks_old_as_superseded() {
        ...
        let mem2 = mem_repo.create(CreateMemoryInput {
            project_id: project_id.clone(),
            memory_type: MemoryType::Decision,
            title: "Use OAuth".into(),
            content: serde_json::json!({}),
            confidence: None, source: None, ai_assisted: None,
        }).unwrap();

        let repo = SqliteDecisionRepository::new(store);
        let old = repo.create(make_decision_input(project_id.clone(), memory_id)).unwrap();

...
impl SqliteEventRepository {
    ...
    pub fn list_by_type(
        ...
        let rows = stmt
            .query_map(params![event_type, since, limit as i64], row_to_event)
            .map_err(AresError::db)?;
...
mod tests {
    ...
    use ares_core::{new_id, Project, ProjectMaturity};

    fn setup_project(store: &Store) -> ProjectId {
        let now = now_micros();
        let id = ProjectId::new();
        SqliteProjectRepository::new(store.clone())
            .create(&Project {
                id: id.clone(),
                name: "ev-test".into(),
                description: "".into(),
    ...
    fn events_table_is_append_only() {
        ...
        assert!(
            result.is_err(),
            "DELETE on events should be rejected by trigger"
        );
    }
    ...
    fn list_by_type_filters_correctly() {
        ...
        let decision_events = repo
            .list_by_type("decision.created", before - 1, 10)
            .unwrap();

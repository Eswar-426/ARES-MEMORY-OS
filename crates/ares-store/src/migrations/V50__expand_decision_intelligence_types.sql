-- V50__expand_decision_intelligence_types.sql
-- Expand graph_nodes node_type constraint to include 'review_trigger'
-- Expand graph_edges edge_type constraint to include 'has_alternative', 'has_review_trigger'
...
CREATE TABLE IF NOT EXISTS graph_edges_new (
  ...
  edge_type     TEXT NOT NULL
                CHECK(edge_type IN (
                  ...
                  'has_alternative','has_review_trigger',
                  'drives','satisfies','owned_by',
                  'supported_by','validated_by','contributed_to',
                  'maintains','touches','authored_by','released_in',
                  'evolves','drifts','supports','observes',
                  'member_of','approved_by','reviewed_by'
                )),
  weight        REAL NOT NULL DEFAULT 1.0,
  confidence    REAL NOT NULL DEFAULT 1.0,
  source        TEXT NOT NULL DEFAULT 'scanner'
                CHECK(source IN ('human','scanner','agent','inference','git_commits','git_blame','git_releases','git_branches','codeowners', 'evolution_engine', 'evidence_engine', 'governance_engine')),

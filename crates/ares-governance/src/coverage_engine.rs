use crate::classifier::{ArtifactCategory, ArtifactClassifier, MemoryEligibility};
...
impl CoverageEngine {
    pub fn calculate(
    ...
    ) -> Result<MemoryCoverageMetrics, AresError> {
        ...
        for node in &nodes {
            let classification =
                ArtifactClassifier::classify(Some(&node.node_type), node.file_path.as_deref());

            println!(
                "DEBUG: Node: {} | Category: {:?} | Path: {:?}",
                node.id, classification.category, node.file_path
            );

            // Check Eligibility for the primary "Overall" metric
            if classification.eligibility == MemoryEligibility::Required
                || classification.eligibility == MemoryEligibility::Recommended
            {
                memory_eligible_count += 1;
                ownership_total += 1;

                // Has Owner?
                let has_owner = edges.iter().any(|e| {
                    e.from_node_id == node.id && e.edge_type == ares_core::EdgeType::OwnedBy
                });
                if has_owner {
                    ownership_covered += 1;
                } else {
                    println!(
                        "DEBUG_MISSING_OWNER: Node: {} | Category: {:?} | Path: {:?}",
                        node.id.as_str(),
                ...
                let has_reasoning = edges.iter().any(|e| {
                    e.to_node_id == node.id
                        && (e.edge_type == ares_core::EdgeType::Drives
                            || e.edge_type == ares_core::EdgeType::Satisfies
                            || e.edge_type == ares_core::EdgeType::Implements)
                });
                if has_reasoning
                    || classification.category == ArtifactCategory::Requirement
                    || classification.category == ArtifactCategory::Decision
                    || classification.category == ArtifactCategory::Architecture
                {
            ...
            match classification.category {
                ArtifactCategory::Requirement => {
                    ...
                    let is_linked = edges.iter().any(|e| {
                ...
                ArtifactCategory::Decision => {
                    ...
                    let links_to_code = edges.iter().any(|e| {
                        e.from_node_id == node.id && e.edge_type == ares_core::EdgeType::Drives
                    });
                    let linked_from_req = edges.iter().any(|e| {
                        e.to_node_id == node.id && e.edge_type == ares_core::EdgeType::Drives
                    });
                    if links_to_code && linked_from_req {
                        dec_covered += 1;
                    }

                    // Decision with evidence
                    let has_evidence = edges.iter().any(|e| {
                        e.to_node_id == node.id && e.edge_type == ares_core::EdgeType::SupportedBy
                    });
                    if has_evidence {
                        dec_with_evidence += 1;
                    }
                ...
                ArtifactCategory::Architecture => {
                    ...
                    let is_linked = edges
                        .iter()
                        .any(|e| e.from_node_id == node.id || e.to_node_id == node.id);
                    if is_linked {
                        arch_covered += 1;
                    }
                }
                ArtifactCategory::Code => {
                    code_total += 1;
                    let has_test = edges.iter().any(|e| {
                        e.to_node_id == node.id && e.edge_type == ares_core::EdgeType::ValidatedBy
                    });
                    if has_test {
                        code_with_tests += 1;
                    }
        ...
        let has_commits = nodes
            .iter()
            .any(|n| n.node_type == ares_core::NodeType::Commit);
        let has_releases = nodes
            .iter()
            .any(|n| n.node_type == ares_core::NodeType::Release);
        ...
        let capture_rate = MemoryCaptureRate {
            ...
            rate: if available > 0 {
                (captured as f64 / available as f64) * 100.0
            } else {
                0.0
            },
        };

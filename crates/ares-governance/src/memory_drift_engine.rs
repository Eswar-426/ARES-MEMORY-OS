...
impl MemoryDriftEngine {
    pub fn calculate(
    ...
    ) -> Result<MemoryDriftMetrics, AresError> {
        ...
        for node in &nodes {
            ...
            if classification.eligibility == MemoryEligibility::Required
                && classification.category == ArtifactCategory::Code
            {
                ...
                let upstream_reasoning: Vec<_> = edges
                    .iter()
                    .filter(|e| {
                ...
                if upstream_reasoning.is_empty() {
                    ...
                    let max_reasoning_update = upstream_reasoning
                        .iter()
                        .map(|n| n.updated_at)
                        .max()
                        .unwrap_or(0);

                    if node.updated_at > max_reasoning_update
                        && (node.updated_at - max_reasoning_update) > thirty_days_ms
                    {
                        artifacts_changed_without_memory_updates += 1;
                    }
                }
